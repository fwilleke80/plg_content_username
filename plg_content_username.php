...
class PlgContentUsername extends CMSPlugin
{
    ...
    public function onContentPrepare($context, &$article, &$params, $page = 0)
    {
        ...
        Log::add('Triggered onContentPrepare(), context: ' . $context, Log::DEBUG, 'username');
    ...
    private function replacePlaceholders(&$text, $user)
    {
        ...
        if (strpos($text, '{{username}}') !== false)
        {
            ...
            Log::add('Replaced "{{username}}" with "' . $username . '".', Log::DEBUG, 'username');
        ...
        if (strpos($text, '{{username_full}}') !== false)
        {
            ...
            Log::add('Replaced "{{username_full}}" with "' . $usernameFull . '".', Log::DEBUG, 'username');
